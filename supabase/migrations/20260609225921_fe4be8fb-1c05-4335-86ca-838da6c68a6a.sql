...
CREATE OR REPLACE FUNCTION public.admin_audit(p_actor uuid, p_action text, p_target_type text, p_target_id uuid, p_detail jsonb DEFAULT '{}'::jsonb)
...
CREATE OR REPLACE FUNCTION public.admin_review_request(p_type text, p_id uuid, p_approve boolean, p_note text DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.admin_adjust_balance(p_user uuid, p_amount numeric, p_memo text DEFAULT NULL)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v_admin uuid := auth.uid(); v_wallet uuid;
BEGIN
  IF v_admin IS NULL OR NOT public.is_admin(v_admin) THEN RAISE EXCEPTION 'Admin only'; END IF;
  ...
  IF v_wallet IS NULL THEN RAISE EXCEPTION 'Primary wallet not found'; END IF;
  PERFORM public.wallet_adjust(v_wallet, p_amount,
  ...
  PERFORM public.admin_audit(v_admin,'balance_adjusted','user',p_user, jsonb_build_object('amount',p_amount,'memo',p_memo));
END $$;
...
BEGIN
  ...
  PERFORM public.admin_audit(v_admin, CASE WHEN p_frozen THEN 'farmer_frozen' ELSE 'farmer_unfrozen' END,'user',p_user,'{}'::jsonb);
END $$;

CREATE OR REPLACE FUNCTION public.admin_cancel_cycle(p_cycle_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v_admin uuid := auth.uid(); v_c public.cycles%ROWTYPE; v_wallet uuid;
BEGIN
  IF v_admin IS NULL OR NOT public.is_admin(v_admin) THEN RAISE EXCEPTION 'Admin only'; END IF;
  ...
  PERFORM public.wallet_adjust(v_wallet, v_c.amount,'adjustment'::ledger_kind,'Cycle cancelled by admin - principal refunded','cycles',p_cycle_id);
  PERFORM public.admin_audit(v_admin,'cycle_cancelled','cycle',p_cycle_id, jsonb_build_object('amount',v_c.amount,'user_id',v_c.user_id));
END $$;

CREATE OR REPLACE FUNCTION public.admin_force_mature_cycle(p_cycle_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v_admin uuid := auth.uid(); v_c public.cycles%ROWTYPE;
BEGIN
  IF v_admin IS NULL OR NOT public.is_admin(v_admin) THEN RAISE EXCEPTION 'Admin only'; END IF;
  SELECT * INTO v_c FROM public.cycles WHERE id=p_cycle_id FOR UPDATE;
  IF v_c.id IS NULL THEN RAISE EXCEPTION 'Cycle not found'; END IF;
  ...
  PERFORM public.admin_audit(v_admin,'cycle_matured','cycle',p_cycle_id, jsonb_build_object('user_id',v_c.user_id));
END $$;

CREATE OR REPLACE FUNCTION public.admin_create_coupon(p_code text, p_amount numeric, p_max integer, p_expires timestamptz DEFAULT NULL)
...
BEGIN
  ...
  VALUES (v_code, p_amount, p_max, p_expires, true, v_admin) RETURNING id INTO v_id;
  PERFORM public.admin_audit(v_admin,'coupon_created','coupon',v_id, jsonb_build_object('code',v_code,'amount',p_amount,'max',p_max));
  RETURN v_id;
END $$;
...
BEGIN
  ...
  PERFORM public.admin_audit(v_admin, CASE WHEN p_active THEN 'coupon_enabled' ELSE 'coupon_disabled' END,'coupon',p_id,'{}'::jsonb);
END $$;
