-- =========================================================
-- Phase 6: Escrow Trades
-- Table + enum + grants + RLS + SECURITY DEFINER lifecycle RPCs
-- (follows the convention of 20260608001243 — DDL + grants + RLS + RPCs
--  in ONE file). Money movement reuses the service-role-only
--  wallet_adjust() mutator and the existing escrow_* ledger kinds.
-- =========================================================

-- 1. Status enum ------------------------------------------------------------
DO $$ BEGIN
  CREATE TYPE public.escrow_status AS ENUM (
    'pending',    -- funded by payer, awaiting payee acceptance
    'accepted',   -- payee accepted the terms
    'released',   -- payer (or admin) released funds to payee  [terminal]
    'cancelled',  -- cancelled before acceptance, refunded to payer [terminal]
    'disputed',   -- a participant opened a dispute, awaiting admin
    'refunded'    -- admin refunded the payer [terminal]
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 2. escrow_trades table ----------------------------------------------------
...
-- Participants (payer or payee) and admins may read; never anon.
...
-- 3. RPC: escrow_create -----------------------------------------------------
-- Payer funds the escrow immediately: the amount is debited from the payer's
-- primary wallet (escrow_lock) and held by the trade until release/refund.
...
BEGIN
  ...
  -- Hold the funds: debit the payer's primary wallet now.
  PERFORM public.wallet_adjust(
    v_pw.id, -p_amount, 'escrow_lock'::ledger_kind,
    'Escrow funded' || COALESCE(': ' || NULLIF(trim(p_title), ''), ''),
    'escrow_trades', v_id
  );
...
DECLARE
  ...
  v_e    public.escrow_trades%ROWTYPE;
BEGIN
...
-- 5. RPC: escrow_release ----------------------------------------------------
-- Payer releases the held funds to the payee. Allowed from pending or accepted.
...
BEGIN
  ...
  PERFORM public.wallet_adjust(
    v_pw.id, v_e.amount, 'escrow_release'::ledger_kind,
    'Escrow released', 'escrow_trades', v_e.id
  );
END $$;

-- 6. RPC: escrow_cancel -----------------------------------------------------
-- Either participant may cancel while still pending; funds are refunded to payer.
...
BEGIN
  ...
  PERFORM public.wallet_adjust(
    v_pw.id, v_e.amount, 'escrow_refund'::ledger_kind,
    'Escrow cancelled — refund', 'escrow_trades', v_e.id
  );
END $$;

-- 7. RPC: escrow_dispute ----------------------------------------------------
CREATE OR REPLACE FUNCTION public.escrow_dispute(p_id uuid, p_reason text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- 8. RPC: escrow_resolve (admin only) --------------------------------------
-- Admin resolves a disputed escrow: release to payee, or refund the payer.
CREATE OR REPLACE FUNCTION public.escrow_resolve(p_id uuid, p_release boolean, p_resolution text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF p_release THEN
    ...
    IF v_pw.id IS NULL THEN RAISE EXCEPTION 'Counterparty primary wallet not found'; END IF;
    UPDATE public.escrow_trades
    SET status = 'released', resolution = NULLIF(trim(p_resolution), ''), resolved_by = v_user, updated_at = now()
    WHERE id = p_id;
    PERFORM public.wallet_adjust(v_pw.id, v_e.amount, 'escrow_release'::ledger_kind,
      'Escrow released by admin', 'escrow_trades', v_e.id);
  ELSE
    SELECT * INTO v_pw FROM public.wallets WHERE user_id = v_e.payer_id AND kind = 'primary';
    IF v_pw.id IS NULL THEN RAISE EXCEPTION 'Payer primary wallet not found'; END IF;
    UPDATE public.escrow_trades
    SET status = 'refunded', resolution = NULLIF(trim(p_resolution), ''), resolved_by = v_user, updated_at = now()
    WHERE id = p_id;
    PERFORM public.wallet_adjust(v_pw.id, v_e.amount, 'escrow_refund'::ledger_kind,
      'Escrow refunded by admin', 'escrow_trades', v_e.id);
  END IF;
END $$;

-- 9. Grants on the RPCs -----------------------------------------------------
-- Authenticated callers invoke these from server functions; each function
-- enforces its own authorization via auth.uid()/is_admin() internally.
